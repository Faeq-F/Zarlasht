import app/actors/actor_types.{
  type PlayerSocket, type WebsocketActorState, EnqueueParticipant, Two,
  WebsocketActorState,
}
      ...
      case waiting_games |> table.lookup(code) {
        ...
        _ -> {
          ...
          process.send(
            ...
            EnqueueParticipant(code, Two, player.state.ws_subject),
          )
          WebsocketActorState(..player.state, player: Two)
        }
      }
    }
    _ -> {
