//// The handler for a player clicking on a position on the map
...
/// The handler for the ClickedPosition message
///
pub fn clicked_position(position: String, state: WebsocketActorState) {
  ...
  //let action = Move(0)
  let action_message = case action {
    Move(_) -> "You should continue moving"
    Battle(_, _, _, _) -> "You need to fight!"
  }
  //update game state
  let player =
    Player(
      ...
      updates: state.player.updates
        |> list.append([movement_message, action_message]),
    )
